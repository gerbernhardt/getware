...
$sql = 'SELECT x.name FROM `sys~admin~privileges~types` AS x';
...
if(isset($_GET['admin']) && file_exists($_ADMIN['path'] . $_GET['admin'] . '.php')) {
    ...
    $sql = 'SELECT x.id FROM `sys~admin` AS x WHERE x.file=\'' . $_GET['admin'] . '\' AND x.access = 1';
    if($result = mysqli_query($_DB['session'], $sql)) {
        if($fetch = $result->fetch_array()) {
            ...
            if($_USER['type'] != 1) {
                ...
                $sql = 'SELECT x.* FROM `sys~admin~privileges` AS x WHERE x.user = ' . $_USER['id'] . ' AND x.module = \'' . $fetch['id'] . '\'';
                if($result = mysqli_query($_DB['session'], $sql)) {
                    if($fetch = $result->fetch_array()) {
                        $_ADMIN['file'] = true;//TIENE ACCESO AL MODULO
                        $sql = 'SELECT DISTINCT x1.name FROM `sys~admin~privileges` AS x';
                        $sql .= ' INNER JOIN `sys~admin~privileges~types~data` AS x0 ON x0.privilege=x.id';
                        $sql .= ' INNER JOIN `sys~admin~privileges~types` AS x1 ON x0.type=x1.id';
