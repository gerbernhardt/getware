...
# Filename: kernel/grid.edit.save.php,2004/09/06
...
if(isset($_GET['save'])&&($_ADMIN['edit']==true)) {
 ...
 for($i=0;$i<count($_GET['save']);$i++) {
  ...
  for($j=0;$j<count($row['name']);$j++) {
   if(!isset($_POST[$row['name'][$j]][$i])) $_POST[$row['name'][$j]][$i]='';
    ...
    } else $set.='x.'.$row['name'][$j].'=\''.$_POST[$row['name'][$j]][$i].'\'';
    if($j<count($row['name'])-1) $set.=',';
  }
  $sql.=$set.' WHERE x.id='.$_GET['save'][$i].' AND '.$_MODULE['restrict'][_EDIT];
  $rows.='"'.$_GET['save'][$i].'"';
  if(mysqli_query($_DB['session'],$sql)) {
   ...
   ////// end of insert
  } else {$data.='"0"';unset($affected_rows[$i]);}
