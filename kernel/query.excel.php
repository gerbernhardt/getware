 ...
 * Filename: kernel/query.excel.php
 * by German Bernhardt
...
if(isset($_GET['excel'])) {
    $sql = 'SELECT x.sql FROM `sys~query` AS x';
    ...
    if($result = mysqli_query($_DB['session'],$sql)) {
        if($fetch=$result->fetch_array()) {
            ...
            for($i = 0; $i < count($_MODULE['grid']['name']); $i++) {
                ...
                $objPHPExcel->getActiveSheet()->getColumnDimension($col[$i])->setWidth($_MODULE['grid']['size'][$i] * 2);
            }

            $sql = base64_decode($fetch[0]);
            if($no_limits) $sql = substr($sql, 0, -10);
            if($result = mysqli_query($_DB['session'], $sql)) {
