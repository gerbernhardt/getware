...
# Filename: dialog.edit.php, 2012/04/03
...
class kernel_dialog_edit{
 function save() {
  ...
  $path='kernel/dialog.edit.';$files=array('save');eval($CORE->include_enc());
 }
 function show($upload=false){
  ...
  if($result=mysqli_query($_DB['session'],$sql)){
   if(mysqli_num_rows($result)==count($_GET['edit'])){    unset($result);
    $this->save();
    $path='kernel/dialog.edit.';$files=array('show');eval($CORE->include_enc());
