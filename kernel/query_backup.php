  ...
  if($_TABLE['column']['function'][$x]=='REFERENCE') {
   ...
   for($j=1;$j<count($_TABLE['column']['comment'][$x]);$j++) {
    ...
    if(preg_match('/\[*\]/',$_TABLE['column']['comment'][$x][$j])){
     $xxreg=explode(':',preg_replace('#.*\[(.*?)\]#si','\1',$_TABLE['column']['comment'][$x][$j])); // codigo[articulo:codigos]  1=articulos , 2=nombre
     $xreg=preg_replace('#(.*?)\[.*#si','\1',$_TABLE['column']['comment'][$x][$j]); // codigo[articulo:nombre]  = codigo

     if(preg_match('/\{*\}/',$xxreg[1])){
      $xxxreg=explode(';',preg_replace('#.*\{(.*?)\}#si','\1',$xxreg[1])); // codigo{articulo;codigos}  1=articulos , 2=nombre      $xxreg[1]=preg_replace('#(.*?)\{.*#si','\1',$xxreg[1]);  // codigo{articulo;nombre}  = codigo

      $_MODULE['query']['select'].='(SELECT xxx.'.$xxxreg[1].' FROM '.$xxxreg[0].' AS xxx';
      $_MODULE['query']['select'].=' INNER JOIN '.$xxreg[0].' as xx ON xx.'.$xxreg[1].'=xxx.id';
      $_MODULE['query']['select'].=' WHERE x'.$as.'.'.$xreg.'=xx.id)';

     }else $_MODULE['query']['select'].='(SELECT xx.'.$xxreg[1].' FROM '.$xxreg[0].' AS xx WHERE x'.$as.'.'.$xreg.'=xx.id)';
   ...
   if($_TABLE['column']['function'][$x]=='REFERENCE') {
    ...
    for($j=1;$j<count($_TABLE['column']['comment'][$x]);$j++) {
     ...
     if(preg_match('/\[*\]/',$_TABLE['column']['comment'][$x][$j])){
      ...
      $field.='(SELECT xx.'.$subreg[1].' FROM '.$subreg[0].' AS xx WHERE x'.$as.'.'.$reg.'=xx.id)';
