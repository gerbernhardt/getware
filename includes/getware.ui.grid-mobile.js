 ...
 * Filename: includes/getware.ui.grid-mobile.js
 * by German Bernhardt
...
getware.ui.grid={
 ...
 toolbar:function(module,date,options){
  ...
  for(i=0;i<options.data.length;i++){
   ...
   output+='<div style="margin-left:' + ((64*i) + 10) + 'px;" class="icon-toolbar icon-'+options.img[i]+'">&nbsp;</div></a>';
 ...
 check:function(date){
  ...
  $(h+' input').click(function(){
   ...
   if(this.checked==true){ // ALL CKECK ON
    ...
    .find('input').each(function(){this.checked=false;});
   }
  });
 ...
 make:function(json){
   ...
   */
  output+='<div id="title">';
  output+='<img class="grid-title-menu" src="images/down-mobile.png" onclick="javascript:getware.ui.menu.make(\''+json.module+'\','+date+','+json.menu+');">';
  output+=json.title;
  output+='</div>';
  /*
   * TOOLBAR
  ...
  // CHECKBOX ALL
  output+='<div id="row-0-'+date+'"><input class="grid-checkbox" id="0-'+date+'" type="checkbox" /></div>';
   ...
   */
   ...
   // CHECKBOX INDIVIDUAL
   output+='<div id="row-0-'+date+'"><input class="grid-checkbox" id="'+json.data[i][0]+'-'+date+'" type="checkbox" /></div>';
   ...
   */
   ...
   // PREV - FISRT
   output+='<a onclick="javascript:getware.get(\'module=admin&amp;admin='+json.module+'&amp;start=0\',getware.ui.search.form(\'form_search_'+date+'\'));" href="#"><img src="images/navbar_first.png"></a>';
   prev=json.navbar.start-1;
   if(prev<0) prev=0;
   output+='<a onclick="javascript:getware.get(\'module=admin&amp;admin='+json.module+'&amp;start='+prev+'\',getware.ui.search.form(\'form_search_'+date+'\'));" href="#"><img src="images/navbar_prev.png"></a>';
   /*
   ...
   */
   // NUMBERS
   if(pages>5&&json.navbar.start>2){
    if(json.navbar.start+2<pages)
     ini=json.navbar.start-1;
    else ini=pages-5;
   }else ini=0;

   for(i=0;i<5&&ini<pages;i++){
   ...
   output+='<a onclick="javascript:getware.get(\'module=admin&amp;admin='+json.module+'&amp;start='+next+'\',getware.ui.search.form(\'form_search_'+date+'\'));" href="#"><img src="images/navbar_next.png"></a>';
   output+='<a onclick="javascript:getware.get(\'module=admin&amp;admin='+json.module+'&amp;start='+(pages-1)+'\',getware.ui.search.form(\'form_search_'+date+'\'));" href="#"><img src="images/navbar_last.png"></a>';
 ...
 init:function(date,json){
  ...
  for(i=1;i<$(ghlr).length;i++){
   ...
   r.css('width',columns[i-1].width*3);
   rc.css('width',columns[i-1].width*3);
  ...
  */
  ...
  $(g+' div li div[id^=row-0]').css('width',70);
  $(g+' div li div[id^=row-0]').css('width',70);
  $(g+' div li div[id*=row-0]').css('height',70);
  $(g+' div li div[id*=row-0]').css('height',70);
 }
}