...
// Filename: includes/getware.ui.content.js, 2012/04/03
...
$getware.ui.content={
 aff:{
  $id:[],
  $inner:'x',
  dblclick:function($window,$this){
   var $split=$this.split('x');
   if($split.length>1){
    if($split[1]>1){
     var $upValue=$('div[id=ui-window-'+$window+'] #'+$split[0]+'x'+($split[1]-1)).val();
     $('div[id=ui-window-'+$window+'] #'+$this).val($upValue);
    }
   }
  },
  add:function($window){
   if(!$getware.ui.content.aff.$id[$window])
    $getware.ui.content.aff.$id[$window]=0;
