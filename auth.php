...
$sql='DELETE FROM sys_sessions AS x WHERE x.time<='.time();
...
function login() {
 ...
 $sql='SELECT x.* FROM sys_users AS x';
 ...
 if($result=mysqli_query($_DB['session'],$sql)) {
  if($_USER=$result->fetch_array()) {
   # DELETE OLD SESSION
   $sql='DELETE FROM sys_session';
   $sql.=' WHERE ip=\''.$_SERVER['REMOTE_ADDR'].'\'';
   $sql.=' AND user='.$_USER['id'];
   mysqli_query($_DB['session'],$sql);
   # INSERT NEW SESSION
   $sql='INSERT INTO sys_sessions (`user`,`key`,`time`,`ip`)';
...
function session() {
 global $_DB,$_USER;
 $sql='SELECT x.* FROM sys_sessions AS x';
